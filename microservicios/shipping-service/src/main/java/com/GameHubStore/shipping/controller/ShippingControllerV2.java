...
@RestController
@RequestMapping("/api/v2/shippings")
...
public class ShippingControllerV2 {
    ...
    public ResponseEntity<EntityModel<ShippingResponse>> createShipping(@Valid @RequestBody ShippingRequest request) {
    ...
    public ResponseEntity<CollectionModel<EntityModel<ShippingResponse>>> getShippings(
        ...
        List<EntityModel<ShippingResponse>> shippings = shippingService.getShippings(ordenId, estado).stream()
                .map(assembler::toModel)
                .collect(Collectors.toList());

        return ResponseEntity.ok(CollectionModel.of(shippings,
                linkTo(methodOn(ShippingController.class).getShippings(ordenId, estado)).withSelfRel()));
    }
    ...
    public ResponseEntity<EntityModel<ShippingResponse>> getShippingById(
    ...
    public ResponseEntity<EntityModel<ShippingResponse>> updateShippingStatus(
            @Parameter(description = "ID del despacho", required = true, example = "1")
            @PathVariable Long id,
        ...
        ShippingResponse response = shippingService.updateShippingStatus(id, estado, tracking);
        return ResponseEntity.ok(assembler.toModel(response));
    }
